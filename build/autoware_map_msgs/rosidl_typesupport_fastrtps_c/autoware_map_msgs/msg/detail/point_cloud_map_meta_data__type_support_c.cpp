  ...
  void * untyped_ros_message)
{
  ...
  // Field name: metadata_list
  {
    ...
    if (!autoware_map_msgs__msg__PointCloudMapCellMetaDataWithID__Sequence__init(&ros_message->metadata_list, size)) {
      fprintf(stderr, "failed to create array for field 'metadata_list'");
      return false;
    }
    auto array_ptr = ros_message->metadata_list.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = autoware_map_msgs__msg__PointCloudMapMetaData;
    is_plain =
      (
...
static size_t _PointCloudMapMetaData__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_autoware_map_msgs__msg__PointCloudMapMetaData(
